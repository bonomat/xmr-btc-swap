...
mod tor_test {
    ...
    use std::{convert::Infallible, env, fs, fs::File};
    ...
    fn run_tmp_tor() -> Result<(Option<AutoKillChild>, u16, u16, Option<NamedTempFile>)> {
        ...
        let _file = File::open(torrc_file.clone())?;
        ...
        .expect("Starting tor failed");
        tracing::info!("Tor running with pid: {}", child.id());
        let child = AutoKillChild::new(child);
        Ok((Some(child), control_port, proxy_port, Some(temp_torrc)))
    }
    ...
    async fn test_tor_control_port() -> Result<()> {
        ...
        // start tmp tor
        let x = env::var("USE_LOCAL_TOR").map_or(false, |val| val.parse().unwrap());

        let (_child, control_port, proxy_port, _torrc_file) = if x {
            (None, 9051, 9050, None)
        } else {
            run_tmp_tor()?
        };
