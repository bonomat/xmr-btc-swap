...
- Tor support as an optional feature. If ASB detects that Tor's control port is open, an hidden service is created for
  network it is listening on (currently 2). The Tor control port as well as Tor socks5 proxy port is configurable.
